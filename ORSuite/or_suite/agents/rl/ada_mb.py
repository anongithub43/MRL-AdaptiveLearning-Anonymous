...
class AdaptiveDiscretizationMB(Agent):
    ...
    def update_obs(self, obs, action, reward, newObs, timestep, info, last=False):
        ...
        # print('active node, ', active_node.bounds, active_node.qVal)
        ...
        if self.flag == False:  # we are doing one-step updates for the estimates
            if timestep == self.epLen - 1:  # q value estimate at last step is straightforward
                ...
                # if reward > 0: 
                #     print('qVal: ', active_node.qVal)
                #     print('next vEst: ', next_tree.vEst)
                #     print('vEst', vEst)
                #     print('rEst', active_node.rEst)
                #     print('scaling ', self.scaling / np.sqrt(active_node.num_visits))
                #print('last term: ', active_node.rEst + vEst + self.scaling / np.sqrt(active_node.num_visits))
                active_node.qVal = min(active_node.qVal, self.epLen, active_node.rEst +
                                       vEst + self.scaling / np.sqrt(active_node.num_visits))
                # if reward > 0: 
                #     print('next val ', active_node.qVal)
            # Update estimate of value function  for state leaves
            index = 0
            for state_val in tree.state_leaves:
                #print('state val: ', state_val)
                if last: 
                    _, qMax = tree.get_active_ball(state_val, to_print=True)
                else: 
                    _, qMax = tree.get_active_ball(state_val, to_print=False)
                #print('qMax: ', qMax)
                tree.vEst[index] = min(qMax, self.epLen, tree.vEst[index])
                #print('tree vEst: ', tree.vEst[index])
                index += 1
    ...
    def pick_action(self, state, timestep):
        ...
        #print('bounds: ', active_node.bounds)
        #print('dim: ', active_node.bounds[action_dim:, :])
