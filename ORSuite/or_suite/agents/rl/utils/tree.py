...
class Tree():
    ...
    def get_active_ball(self, state, node = None, root = True, to_print = False):
        """
        ...
        if len(node.children) == 0:
            ...
            for child in node.children:
                if child.contains(state):
                    nn, nn_qVal = self.get_active_ball(state, child, False, to_print=to_print)
                    #if to_print: 
                    #    print('child and qval', nn.bounds, nn_qVal)
                    #    print(' ----   ')
