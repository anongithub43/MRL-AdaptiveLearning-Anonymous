        ...
        for i in range(self.num_iters):  # loops over the numer of iterations
            ...
            for ep in range(0, self.nEps):  # loops over the episodes
                ...
                while (not done) and h < self.epLen:
                    ...
                    if inState: 
                        df = df.append({'ID': int(cur_id + ep), 
                                        'TIME': int(t), 
                                        'FEATURE_0': oldState[0] * 5, # because the maze is normally [0,4]
                                        'FEATURE_1': -oldState[1] * 5, 
                                        'ACTION': real_action, 
                                        'RISK': reward}, ignore_index=True) # reward * 10 to force 0.04
                    else: 
                        inState = True
                    ...
                    self.agent.update_obs(
                        oldState, action, reward, newState, h, info, last= (ep >= (self.nEps - 2)))
                ...
                if mrl_flag and (ep - 1) % 200 == 0:
                    ...
                    print('solving maze MDP')

                    true_v, true_pi = SolveMDP(P, R, prob='max', gamma=0.99, epsilon=1e-8)

                    print('getting mrl model opt gap')

                    opt_gap = value_diff([mrl_model], [0], K, T_max, P, R, f, rw, true_v, true_pi) 

                    print('getting opt gap adaptive')

                    opt_gap_adaptive = value_diff([self.agent], [0], K, T_max, P, R, f, rw, true_v, true_pi, adaptive=True)

                    print('done with opt gap adaptive')

                    epGap_adaptive = opt_gap_adaptive[0] / 0.04 + epGap_adaptive
                ...
                if ep % 20 == 0: 
                    ...
                    P, R = get_maze_MDP(maze)
                    f, rw = get_maze_transition_reward(maze)
                    true_v, true_pi = SolveMDP(P, R, prob='max', gamma=0.9, epsilon=1e-8)

                    opt_gap_adaptive = value_diff([self.agent], [0], K, T_max, P, R, f, rw, true_v, true_pi, adaptive=True)

                    epGap_adaptive = opt_gap_adaptive[0] / 0.04 + epGap_adaptive

                df.to_csv('data/trials-updated.csv')
                ...
                self.data[index, 3] = epGap_adaptive
